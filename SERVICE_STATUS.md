...
| SOLID (DIP) Structure | 74% | In Progress | Controllers/consumers now depend on service interfaces in active modules, including user-service (`UserController` -> `UserUseCases`), review-service (`ReviewController` -> `ReviewUseCases`), and cart-service (`CartController` -> `CartUseCases`) with SRP store/resolver decomposition. | Continue SRP split of remaining high-complexity services and formalize orchestration patterns. |
...
5. Raise SOLID maturity from DIP baseline by SRP split of high-complexity services.
