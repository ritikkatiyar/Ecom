...
|---|---:|---|---|---|
| API Gateway | 97% | In Progress | Request logging, JWT policy, correlation ID, rate limit (Redis + in-memory fallback), circuit breakers with all-methods fallback, auth validate timeout 15s, 401/503 logging, SRP split (`GatewayAuthRoutePolicy`, `AuthValidationClient`). | Breaker-threshold tuning by traffic profile and production receiver/alert mapping drills. |
| Auth Service | 85% | In Progress | JWT + refresh + blacklist + OAuth2 baseline implemented; token blacklist uses `common-redis` fallback when Redis unavailable; DIP/SRP split (`AuthUseCases`, `AuthTokenIssuer`, `RefreshTokenGenerator`); request logging via `common-web`. | Token rotation hardening, audits, integration tests, gateway policy hookup. |
...
| Product Service | 75% | In Progress | CRUD + pagination/filter/sort on MongoDB; Cloudinary image upload (10MB limit), Product `imageUrls`, admin ProductForm image picker, multipart 400 logging. | Variants depth, indexing events, stricter validation/versioning. |
...
| Frontend Beta Track | 90% | In Progress | Next.js storefront (`ecom-storefront`) is active with stitch design (Voluspa style), cookie-backed auth refresh flow, live public routes (`/`, `/shop`, `/products/[id]`, `/search`, `/collections`, `/cart`, `/checkout`, `/feedback`), live account routes (`/account/*`), and live admin routes mapped to backend APIs. Feature flags (beta banner, admin-console gating) from gateway; shop infinite scroll; Add-to-Cart; product delete; image upload retry + auto-dismiss. | Execute Phase F hardening + staging smoke gate for beta release. |
...
- Remaining MySQL services (`cart-service`, `user-service`, `review-service`, `notification-service`) are pending Liquibase rollout. `auth-service` adopted Liquibase 2026-03-08.
