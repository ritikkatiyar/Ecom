...
| SOLID (DIP) Structure | 82% | In Progress | Controllers/consumers now depend on service interfaces in active modules, including user-service (`UserController` -> `UserUseCases`), review-service (`ReviewController` -> `ReviewUseCases`), cart-service (`CartController` -> `CartUseCases`), order-service SRP decomposition (`OrderItemCodec`, `OrderResponseMapper`, `OrderEventPublisher`), and payment-service SRP decomposition (`ProviderPaymentIdAllocator`, `PaymentResultPublisher`, `PaymentResponseMapper`). | Continue SRP split of remaining high-complexity services and formalize orchestration patterns. |
...
5. Raise SOLID maturity from DIP baseline by SRP split of remaining high-complexity services (gateway/auth cleanup next).
